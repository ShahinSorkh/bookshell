...
if (isset($_POST['submit']) && $_POST['submit'] === 'new-book') {
    ...
    if ($name && $description && $price && ($cover = save_uploaded_file('cover', 'books/' . md5($name)))) {
        ...
        if (!$result) redirect('/admin/index.php?page=new-book', mysqli_error($db), 'danger');
        else redirect('/admin/index.php?page=new-book', 'book saved', 'success');
    } else redirect('/admin/index.php?page=new-book', 'invalid input', 'danger');
}
?>
