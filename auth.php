...
if (in_array($action, ['login', 'register'])) {
    ...
    if (!$username) {
        redirect('login.php', 'invalid username', 'danger');
    } elseif (!$password) {
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            redirect('login.php', 'user not found', 'danger');
        }
    } else {
        // register
        ...
        if ($password !== $repassword) {
            redirect('login.php', 'passwords do not match', 'danger');
    ...
    redirect('login.php', 'invalid action', 'danger');
}
