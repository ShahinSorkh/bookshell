...
if (!$result) {
    redirect("/book.php?id=$book_id", mysqli_error($db), 'danger');
}
if (mysqli_num_rows($result) < 1) {
    redirect("/book.php?id=$book_id", 'book not found', 'danger');
}
...
if (!$result) {
    redirect("/book.php?id=$book[id]", mysqli_error($db), 'danger');
}
